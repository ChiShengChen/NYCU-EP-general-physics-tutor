    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="max-w-4xl mx-auto space-y-4">
          {/* Filters */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 space-y-3">
            <input
              value={q}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                    kind === k
                      ...
                      : "bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:border-indigo-700"
                  }`}
                >
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-[11px] text-slate-500 dark:text-slate-400 mr-1">章節：</span>
              <button
          ...
          {/* Results */}
          {loading ? (
            <div className="text-center text-slate-400 dark:text-slate-500 py-8">搜尋中...</div>
            ...
            <div className="space-y-4">
              {groups.map((g) => (
                <div key={g.chapter} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
                    ...
                    {g.items.map((it) => (
                      <li key={it.id} className="px-4 py-3 space-y-1">
                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                          <span className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800">P. {it.page_number}</span>
                          {it.section_title && <span className="truncate">{it.section_title}</span>}
                          <span className="ml-auto text-[10px] text-slate-400 dark:text-slate-500">{it.content_type}</span>
                        </div>
