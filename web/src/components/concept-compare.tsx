  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 bg-white shrink-0">
        ...
        <h1 className="text-lg font-semibold text-slate-800">概念對比</h1>
        <span className="text-xs text-slate-400 ml-auto">discrimination learning</span>
      </header>

      <div className="flex-1 overflow-y-auto px-4 py-5">
        <div className="max-w-4xl mx-auto space-y-5">
          <p className="text-xs text-slate-600 bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2 leading-relaxed">
          ...
          <form onSubmit={submit} className="grid sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-600">概念 A</label>
              <input
                value={a}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-600">概念 B</label>
              <input
                value={b}
          ...
          <div className="space-y-3">
            <p className="text-xs text-slate-500">點以下標籤快速填入欄位（先點 A 欄想要的，再點 B 欄想要的）</p>
            {groups.map((g) => (
              <div key={g}>
                <div className="text-[11px] font-semibold text-slate-500 mb-1.5">{g}</div>
                <div className="flex flex-wrap gap-1.5">
                  {SUGGESTIONS.filter((s) => s.group === g).map((s) => (
                    <div key={s.label} className="inline-flex rounded-full border border-slate-200 bg-white overflow-hidden">
                      <button
                        onClick={() => pickInto("a", s.label)}
                        className="px-2 py-0.5 text-xs text-indigo-700 hover:bg-indigo-50"
                        title={`填入 A 欄：${s.label}`}
                      >
                        A
                      </button>
                      <span className="px-2 py-0.5 text-xs text-slate-700 border-x border-slate-200">{s.label}</span>
                      <button
                        onClick={() => pickInto("b", s.label)}
                        className="px-2 py-0.5 text-xs text-rose-700 hover:bg-rose-50"
    ...
    <div className="space-y-5 pt-2">
      ...
      <div className="text-center">
        <h2 className="text-lg font-semibold text-slate-800">
          <span className="text-indigo-700">{pair.a}</span>
          {" vs "}
          <span className="text-rose-700">{pair.b}</span>
        </h2>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="text-left px-3 py-2 text-xs font-semibold text-slate-500 w-28">維度</th>
              <th className="text-left px-3 py-2 text-xs font-semibold text-indigo-700">{pair.a}</th>
              <th className="text-left px-3 py-2 text-xs font-semibold text-rose-700">{pair.b}</th>
            </tr>
          </thead>
          <tbody>
            {result.rows.map((r, idx) => (
              <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                <td className="align-top px-3 py-2 text-xs font-medium text-slate-600 border-b border-slate-100">
                  {r.dimension}
                </td>
                <td className="align-top px-3 py-2 text-slate-800 border-b border-slate-100">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <div className="bg-white border border-emerald-200 rounded-2xl p-4 space-y-2">
          <h3 className="text-sm font-semibold text-emerald-700">🟰 相似處</h3>
        ...
        <div className="bg-white border border-indigo-200 rounded-2xl p-4 space-y-2">
          <h3 className="text-sm font-semibold text-indigo-700">↔ 關鍵差異</h3>
          <ul className="space-y-1.5 text-xs text-slate-700 list-disc pl-4">
        ...
        <div className="bg-white border border-rose-200 rounded-2xl p-4 space-y-2">
          <h3 className="text-sm font-semibold text-rose-700">⚠️ 常見搞混</h3>
          <ul className="space-y-1.5 text-xs text-slate-700 list-disc pl-4">
