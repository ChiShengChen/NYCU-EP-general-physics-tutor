  ...
  return (
    <div className="flex flex-col h-screen">
      <header className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 bg-white shrink-0">
        ...
        <h1 className="text-lg font-semibold text-slate-800">教 AI — 費曼學習法</h1>
        <span className="text-xs text-slate-400 ml-auto">用「教給別人」的方式學最深</span>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <form
            ...
            className="space-y-2"
          >
            <label className="text-sm font-medium text-slate-700">輸入想練的概念（自由輸入）</label>
          ...
          {groups.map((g) => (
            <div key={g}>
              <h3 className="text-xs font-semibold text-slate-500 mb-2">{g}</h3>
              <div className="flex flex-wrap gap-2">
                {SUGGESTED_CONCEPTS.filter((c) => c.group === g).map((c) => (
                  <button
                    key={c.label}
                    onClick={() => onPick(c.label)}
                    className="px-3 py-1.5 text-sm rounded-full border border-indigo-200 text-indigo-700 bg-white hover:bg-indigo-50 transition-colors"
                  >
                    {c.label}
                  </button>
                ))}
  ...
  return (
    <div className="flex flex-col h-screen max-w-3xl mx-auto">
      <header className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 bg-white shrink-0">
        ...
        <span className="text-xl">🎓</span>
        <h1 className="text-lg font-semibold text-slate-800 truncate">教 AI — {concept}</h1>
        <button
          onClick={onChangeConcept}
          className="ml-auto text-xs px-2 py-0.5 rounded-md text-slate-600 hover:text-indigo-600 hover:bg-slate-50 border border-slate-200 transition-colors"
        >
          換概念
        </button>
      </header>
                ...
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${
                    m.role === "user"
                      ...
                      : "bg-amber-50 border border-amber-200 text-slate-800 shadow-sm rounded-bl-sm"
                  }`}
                >
                  {m.role === "user" ? (
                    ...
                    <>
                      <p className="text-[10px] font-medium text-amber-700 mb-1">🐣 學弟/妹</p>
