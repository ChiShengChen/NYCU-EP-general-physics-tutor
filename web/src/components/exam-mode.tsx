  ...
  return (
    <span
      className={`font-mono text-sm font-semibold px-3 py-1 rounded-lg ${
        isUrgent ? "bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-300 animate-pulse" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200"
      }`}
    >
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12 space-y-8">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">考試模擬模式</h2>
        <p className="text-slate-500 dark:text-slate-400">模擬真實考試環境：限時作答、不顯示答案、交卷後統一批改</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">期中考模擬</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Ch01 ~ Ch16（力學、振盪、流體、波動、聲學）</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">50 分鐘 | 7 選擇 + 3 簡答</p>
        </button>
        ...
        >
          <span className="text-3xl mb-2">📗</span>
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">期末考模擬</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Ch17 ~ Ch31（熱學、電磁學、電路）</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">60 分鐘 | 7 選擇 + 3 簡答</p>
        </button>
      </div>

      <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-2xl p-4">
        <h4 className="text-sm font-semibold text-amber-700 dark:text-amber-300 mb-2">📋 考試規則</h4>
...
function LoadingState({ examType }: { examType: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4">
      ...
      <p className="text-sm text-slate-400 dark:text-slate-500">AI 教授正在根據教材內容出題</p>
    </div>
  );
}
...
function GradingState() {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4">
      ...
      <p className="text-slate-600 dark:text-slate-300 font-medium">AI 正在批改你的試卷...</p>
      <p className="text-sm text-slate-400 dark:text-slate-500">統一批改中，請稍候</p>
    </div>
  );
}
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">{exam.title}</h2>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-xs text-slate-400 dark:text-slate-500">{q.points} 分</span>
        ...
        {q.type === "multiple_choice" && q.options ? (
          ...
          <div className="space-y-2">
            <textarea
              ...
              className="w-full rounded-xl border border-slate-300 dark:border-slate-600 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent resize-none h-40"
            />
            <FormulaHelp />
            <HintPanel
              key={`exam-hint-${q.id}`}
              ...
              <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                  >
                    繼續作答
                  </button>
                </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm text-center">
        ...
        <div className="flex items-center justify-center gap-8 mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">等第</p>
          </div>
          <div>
            <div className="text-4xl font-bold mb-1 text-slate-800 dark:text-slate-100">
              {gradeResult.totalScore}<span className="text-lg text-slate-400 dark:text-slate-500">/{gradeResult.maxScore}</span>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">得分（{percentage}%）</p>
          </div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800 rounded-full h-3 overflow-hidden max-w-md mx-auto">
          <div
      ...
      <div className="bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 rounded-2xl p-5">
        <h3 className="text-sm font-semibold text-indigo-700 dark:text-indigo-300 mb-2">💡 AI 批改總評</h3>
        <MarkdownRenderer content={gradeResult.overallFeedback} />
      </div>

      {/* Per-question results */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">逐題詳解</h3>
