  ...
  return (
    <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 text-sm overflow-hidden">
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 text-left text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100/60 transition-colors"
      >
      ...
      {open && (
        <div className="px-3 pb-3 pt-1 space-y-3 border-t border-indigo-100">
          ...
          {SECTIONS.map((s) => (
            <div key={s.title}>
              <div className="text-xs font-semibold text-indigo-700 dark:text-indigo-300 mb-1.5">{s.title}</div>
              <div className="space-y-1">
                {s.rows.map((row) => (
                  <div
                    key={row.src}
                    className="grid grid-cols-1 sm:grid-cols-2 gap-2 px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs"
                  >
                    <div className="flex flex-col gap-0.5 min-w-0">
                      <code className="font-mono text-slate-700 dark:text-slate-200 break-all whitespace-pre-wrap">{row.src}</code>
                      {row.note && <span className="text-[10px] text-slate-400 dark:text-slate-500">{row.note}</span>}
                    </div>
                    <div className="flex items-center min-w-0 overflow-x-auto text-slate-800 dark:text-slate-100">
          ...
          <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-3 py-2 text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed">
            💡 小撇步：簡答題可以混搭一般文字 + 公式，例如：
            <code className="block mt-1 font-mono bg-white dark:bg-slate-900 px-2 py-1 rounded border border-amber-200 dark:border-amber-800">
